  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-[#A8D8C1] bg-white p-6 shadow-lg">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div className="flex items-center gap-4">
            <div className="rounded-xl bg-gradient-to-r from-[#013E43] to-[#005C57] p-3">
          ...
          <button
            type="button"
            onClick={loadData}
            className="inline-flex items-center gap-2 rounded-lg border border-[#A8D8C1] px-4 py-2 text-[#065A57] transition-colors hover:bg-[#F0F7F4]"
          >
            <FiRefreshCw />
            Refresh
          </button>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-xl border-l-4 border-[#013E43] bg-white p-4 shadow-lg">
          <p className="text-sm text-[#065A57]">Support Tickets</p>
          <p className="text-2xl font-bold text-[#013E43]">{tickets.length}</p>
        </div>
        <div className="rounded-xl border-l-4 border-[#02BB31] bg-white p-4 shadow-lg">
          <p className="text-sm text-[#065A57]">Help Requests</p>
          <p className="text-2xl font-bold text-[#02BB31]">{requests.length}</p>
        </div>
        <div className="rounded-xl border-l-4 border-amber-400 bg-white p-4 shadow-lg">
        ...
        <div className="rounded-xl border-l-4 border-blue-400 bg-white p-4 shadow-lg">
      ...
      <div className="rounded-2xl border border-[#A8D8C1] bg-white p-4 shadow-lg">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="inline-flex rounded-xl bg-[#F0F7F4] p-1">
            <button
              type="button"
              onClick={() => {
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search"
                className="w-full rounded-lg border-2 border-[#A8D8C1] py-2 pl-10 pr-3 outline-none focus:border-[#02BB31] sm:w-72"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(event) => setStatusFilter(event.target.value)}
              className="rounded-lg border-2 border-[#A8D8C1] px-3 py-2 outline-none focus:border-[#02BB31]"
            >
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredTickets.length === 0 ? (
            ...
            filteredTickets.map((ticket) => (
              <div
                key={ticket._id}
                className="rounded-2xl border border-[#A8D8C1] bg-white p-5 shadow-lg"
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <select
                      ...
                      className="rounded-lg border-2 border-[#A8D8C1] px-3 py-2 text-sm outline-none focus:border-[#02BB31]"
                    >
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full border border-[#A8D8C1] bg-[#F0F7F4] px-3 py-1 text-xs font-semibold text-[#013E43]">
                        {request.purpose}
                      </span>
                      <span className="text-xs text-[#065A57]">
...
function EmptyState({ label }) {
  return (
    <div className="rounded-2xl border border-dashed border-[#A8D8C1] bg-white p-10 text-center">
