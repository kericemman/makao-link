...
exports.initializeSubscriptionPayment = async (req, res, next) => {
  try {
    ...
    if (selectedPlan.price <= 0) {
      return res
        .status(400)
        .json({ message: "This plan does not require payment" });
    }
...
exports.changeSubscriptionPlan = async (req, res, next) => {
  try {
    ...
    if (currentIndex === -1 || nextIndex === -1) {
      return res
        .status(400)
        .json({ message: "Invalid subscription plan flow" });
    }
    ...
    if (nextIndex < currentIndex) {
      return res
        .status(400)
        .json({ message: "Downgrades are not supported yet" });
    }

    user.subscription.plan = plan;
    user.subscription.status =
      plan === "normal" ? "free" : "pending_payment";
