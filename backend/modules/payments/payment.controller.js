...
const activateSubscription = async ({ userId, plan, reference, amount, currency, customerCode, subscriptionCode }) => {
  ...
  subscription.paystackSubscriptionCode = subscriptionCode || subscription.paystackSubscriptionCode;
...
exports.verifyPaymentByReference = async (req, res, next) => {
  try {
    ...
    console.log("VERIFY SUCCESS:", result.subscription.status, result.subscription.plan);
