...
const RegisterLandlordPage = () => {
  ...
  const nextPath = useMemo(() => getSafeNext(searchParams.get("next") || "/dashboard"), [searchParams]);
  const agentCode = useMemo(() => searchParams.get("agent") || "", [searchParams]);
  ...
  const getPlanDetails = (plan) => {
    const plans = {
      normal: { name: "Normal", price: "Free", limit: "2 properties", color: "text-gray-600", bgColor: "bg-gray-100" },
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-xl border border-[#A8D8C1] p-6">
              ...
              {agentCode ? (
                <div className="mt-4 flex items-center gap-2 rounded-lg border border-[#A8D8C1] bg-[#F0F7F4] p-3">
                  <FaHandshake className="text-[#0D915C]" />
                  <div>
                    <p className="text-sm font-semibold text-[#013E43]">Agent referral applied</p>
                    <p className="text-xs text-[#065A57]">Code {agentCode.toUpperCase()}</p>
                  </div>
                </div>
              ) : null}
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-[#A8D8C1] p-6">
            ...
            <p className="text-center text-sm text-[#065A57] mt-4">
              ...
              <Link to={`/login?next=${encodeURIComponent(nextPath)}`} className="font-semibold text-[#02BB31] hover:text-[#0D915C]">
                Sign in
              </Link>
            </p>
...
function getSafeNext(next) {
  ...
  if (["/login", "/register", "/forgot-password"].includes(next)) return "/dashboard";
  return next;
}
